...
"""
    ...
    script.extend(compute_block.strip() for _ in range(repeats))
...
def run_ibex_script(repo_root: pathlib.Path, build_dir: pathlib.Path, ibex_script: pathlib.Path) -> tuple[float, subprocess.CompletedProcess[str]]:
    ...
    env = dict(**{"IBEX_LIBRARY_PATH": str(plugin_dir)}, **dict(subprocess.os.environ))
...
def main() -> int:
    ...
    if polars_proc.returncode != 0:
        ...
        return 1
    polars_stats = parse_py_benchmark(polars_proc.stdout, "polars")
    ...
    for name in ["ibex", "polars", "pandas"]:
