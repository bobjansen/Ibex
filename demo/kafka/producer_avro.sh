...
REGISTRY_HOSTS="${REGISTRY_HOSTS:-redpanda:8081}"
TOPIC="${TOPIC:-ticks_avro}"
SCHEMA_FILE="${SCHEMA_FILE:-/demo/tick.avsc}"
...
until rpk registry schema list -X registry.hosts="$REGISTRY_HOSTS" >/dev/null 2>&1; do
  sleep 1
done

rpk registry schema create "${TOPIC}-value" --schema "$SCHEMA_FILE" -X registry.hosts="$REGISTRY_HOSTS" >/dev/null 2>&1 || true

echo "tick-producer-avro: publishing Avro to ${TOPIC} via ${BROKERS} using registry ${REGISTRY_HOSTS} (${TICKS_PER_BATCH} ticks/batch, sleep ${SLEEP_SECONDS}s)"
...
while true; do
  ...
  rpk topic produce "$TOPIC" --brokers "$BROKERS" --schema-id topic -f '%v{json}\n' -X registry.hosts="$REGISTRY_HOSTS" \
    <"$batch_file" >/dev/null
  batch_count=$((batch_count + 1))
  tick_count=$((tick_count + TICKS_PER_BATCH))
  if (( batch_count % LOG_EVERY_BATCHES == 0 )); then
    echo "tick-producer-avro: sent ${tick_count} ticks across ${batch_count} batches"
  fi
  rm -f "$batch_file"
  trap - EXIT
  sleep "$SLEEP_SECONDS"
