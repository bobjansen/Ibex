...
#
#  Requires: R -e 'install.packages(c("nycflights13", "ggplot2", "dplyr"))'
# ─────────────────────────────────────────────────────────────────────────────
...
airports <- ibex_tbl(airports)
flights <- ibex_tbl(flights)
planes <- ibex_tbl(planes)
weather <- ibex_tbl(weather)
...
daily <- flights |>
  ...
  as_timeframe("day") |>
  window("7d") |>
  mutate(rolling_7d = mean(avg_dep_delay)) |>
  collect()
