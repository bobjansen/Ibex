...
| [query-shape-conformance-plan.md](query-shape-conformance-plan.md) | Down from +13-16% suite-wide to one query | `eb5231c`'s query-shape realignment cost +13-16% total at SF-2/8c vs the old hand-fused queries. Mechanism 2 (`projected_scan` sees through checked `Ascribe`) and Ascribe-as-Scan-metadata are landed; q06/q13/q15/q14 resolved. q22's separate shape gap is now closed (2026-08-27): demand-safe left/null-marker lowering to anti join, redundant right `distinct` removal, direct parallel substring, and dense integer membership cut the current artifact's 74.3ms to 23.9ms. The remaining query-shape item is q21's distinct-count-replaceable self-join, documented as a known single-query gap rather than an active generic-rewrite project. |
...
| [radix-partitioned-groupby.md](radix-partitioned-groupby.md) | Noted, not built. High-cardinality group-by is memory-bound; radix partitioning remains a q18/general mechanism. Q10 no longer reaches the generic mixed-key ceiling: FD reduction plus discovery-time `First` gathering handles that shape |
