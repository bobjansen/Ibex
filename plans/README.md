...
| [runtime-multithreading-plan.md](runtime-multithreading-plan.md) | Phases 1–3a + Phase 4 items 1–2 landed | Phase 4 item 1 (ungrouped aggregate) and item 2 (categorical group-by) are in; see "Phase 4 status" for the two rules they establish (partition on row count alone; morsels few and large). **3b is deliberately deferred** — its first slice measured as a negative, PDS-H runs at 85–123% CPU, and the website suite times with scans outside the timed region, so 3b cannot move a published number. Next: item 2 for the string/int/generic hash paths and `distinct`, then item 3 hash join (1880ms of the 5.85s recoverable). |
