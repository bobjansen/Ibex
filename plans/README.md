...
| [serial-parity-comparator-plan.md](serial-parity-comparator-plan.md) | **Deliverable landed via `table_compare`.** `runtime::compare_tables` is the authoritative structured predicate (schema, metadata, values, validity, categorical backing) and the structured parity gate runs it in CI; the plan file predates that landing. Residual: a named serial-vs-chunked runner (same query under `IBEX_PARALLEL=0`/`1`, compared structurally) is folded into kernel-pipeline Phase 2's gates |
