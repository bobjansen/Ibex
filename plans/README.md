...
| [query-shape-conformance-plan.md](query-shape-conformance-plan.md) | Investigation complete for the SF-2/8-core regression cluster; no fix landed yet | `eb5231c`'s query-shape realignment (dropping hand-written pushdown to match Polars' benchmark idiom) cost +13% total at SF-2/8c vs the old hand-fused queries, concentrated in q21/q06/q13/q22/q15. Four independent, non-overlapping mechanisms found (schema-blind lower-time join pushdown; `Ascribe` blind spots in two separate scan-chain walkers; a pre-existing fused-decode barrier-stall bug). One fix attempt (eliding checked ascriptions everywhere) tried and reverted — net worse (+25%). q21/q22 are separate, unrelated shape gaps. See the plan for the suggested order of attack. |
