...
| [kernel-pipeline-execution-plan.md](kernel-pipeline-execution-plan.md) | **Phase 1 landed (2026-08-22).** Proposed architectural successor for the execution engine: typed logical IR, physical pipelines, a morsel executor, and a broad precompiled templated kernel library. Explicitly Umbra-like in plan/pipeline structure but **not** a JIT project. Phase 1 adds `physical::plan_physical`/`explain_physical` beside `build_operator` (serial executor; the island remains the parallel mode) with `MaterializedCall` fallback reasons and path-fired counters. Phase 0's comparator deliverable is covered by the existing `table_compare` parity tooling; the contract-doc and corpus items fold into Phase 2. |
...
| Plan | Notes |
|---|---|
| [deferred-probe-streaming-plan.md](deferred-probe-streaming-plan.md) | Parallel, bounded deferred-probe chunks through row-local right-side wrappers; preserves the Parquet boundary and requires multicore q12 validation. Stage 2 landed, was instrumented as unreachable by any current query, and was then removed from the tree (recoverable from `c5ab31ca`); Stage 3 (Filter admission) measured and reverted — fixed q03, broke q12 (no real selectivity, known no-cost-model gap). Blocked on [deferred-probe-selectivity-cost-model-plan.md](deferred-probe-selectivity-cost-model-plan.md). |
| [deferred-probe-selectivity-cost-model-plan.md](deferred-probe-selectivity-cost-model-plan.md) | Scoping only. A real key-domain-overlap estimate for deferred-probe registration, to replace the current row-count-ratio gate — needed before Stage 3 above can be re-attempted. Candidate: trace the build key's origin table via `column_origin_of` and compare its own row count to the build side's estimated (filtered) rows, avoiding three already-documented failure modes (structural filter-proxy, sampled distinct counts, footer-inflated distinct counts on `l_orderkey`-shaped columns). |
