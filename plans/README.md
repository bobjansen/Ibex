...
| [runtime-multithreading-plan.md](runtime-multithreading-plan.md) | Phases 1–3a + Phase 4 items 1–2, item 3 retired, item 4 part-done | Items 1 (ungrouped aggregate) and 2 (categorical group-by) are in; see "Phase 4 status" for the two rules they establish (partition on row count alone; morsels few and large). **3b is deliberately deferred** — its first slice measured as a negative, PDS-H runs at 85–123% CPU, and the website suite times with scans outside the timed region, so 3b cannot move a published number. **Item 3 (hash join) is RETIRED, not scheduled**: the gap was a Categorical probe key being hashed as text per row, and deleting that put 3 of 4 join benches 1.4–6.2× ahead of threaded polars (`7e6576e`). **Item 4's rank consumer is done** (`ff42bd9`, `463fe8a`, `35f1d4f`): 584.7 → 135.3ms, 0.51× → 1.73× of polars. **The 5.85s recoverable breakdown is STALE — re-derive it before ranking anything by it.** Next: item 2 for the string/int/generic hash paths and `distinct` — but profile first, five gaps in a row turned out to be redundant hashing of a dense Categorical code rather than missing parallelism. **Scale caveat: the threading share of a gap grows with data while Ibex's does not.** On a fixed 8 cores, polars' MT gain went 1.69× (1M) → 2.18× (8M) → 2.27× (16M) while Ibex's stayed flat at 1.41/1.43/1.45; the headline ratio erodes 2.50× → 1.92× → 1.85× while the per-core ratio does not move at all (2.99/2.93/2.91). So per-core wins do not hold the headline as data grows — re-measure at the target scale before concluding threading matters less than a small table suggests. |

## Proposed — no implementation yet

| Plan | Notes |
|---|---|
| [pipelined-execution-plan.md](pipelined-execution-plan.md) | **Scoping only.** PDS-H SF-2 thread sweep (1/2/4/8, archived): Ibex is **2.16x faster than polars on ONE core and 1.15x slower on EIGHT**; its first doubling returns 1.28x vs polars' 1.87x, so it is short from the start — not contention. Amdahl fits the whole curve at a **44% parallel fraction** (predicts 1.62x at 8 cores, 1.66x observed); per-query median 41%, 14/22 below 50%. Cause: **materialize-then-fan-out** — every source is drained to one chunk (`chunks=1` everywhere, even though `ChunkedParquetSourceOperator` exists and is immediately materialized), and islands materialize before fanning out by a load-bearing invariant. Hard constraint: pushdown/decode-fusion wins (−12/−14/−27% geomean) all depend on `LazyTable` seeing whole-query demand, so streaming must be built ON them, not instead. Phase 0 (make multi-chunk real — the cross-chunk operator paths have NEVER RUN) is the cheap, high-value first step |
