...
| [per-occurrence-scan-selections-plan.md](per-occurrence-scan-selections-plan.md) | **Fixes an OPEN regression** (bisected to `afe55f25`, 2026-08-31). A source scanned more than once loses ALL filter pushdown: `scan_predicates.cpp:210` keys on source name and cannot tell occurrences apart, so it erases every predicate. Answers stay correct; `ibex-e2e.sh` catches it only because it asserts a profile marker. Lifting the gate is UNSOUND (`like`/`!like` intersect to 0 rows); re-splitting occurrences was built and REJECTED (second decode, breaks `test_repl.cpp:1060`, would hit q21). Design: separate the per-occurrence SELECTION from the per-source DECODE, in 4 phases starting with a no-behaviour-change identity phase. |
