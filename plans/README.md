...
| [dynamic-filter-pushdown-plan.md](dynamic-filter-pushdown-plan.md) | Stages 1-2 done (uncommitted) | Sideways information passing: a join's build keys become {min/max, Bloom, exact IN-list} pushed into the probe side's parquet scan via deferred decode + `project_where` Selection (no plugin ABI change). Stage 2 (Bloom + IN-list + sampled escape hatch) landed 2026-07-19: **suite geomean 0.895** — q08 −47%, q17 −45%, q09 −40%, q03 −20%, q05 −17%, q07 −16%, q18 −11%; lineitem prunes to 0.9-16% of rows, q18 to 399 rows via a 57-key IN-list; hit-heavy stress neutral; q10 +5% accepted (agg-dominated, stage 4 is the fix). 22/22 answers identical, 1207 tests green, g++ strict clean. GOTCHA: the Bloom must front even an exact IN-list — bare binary_search per probe key cost q17 +80% before the flip. Stage 3 (compounding) fires free on chained author orders (measured); stage 4 = fuse the filter into the decoder. Key constraint: filtering must gate materialization, not the hash lookup |
