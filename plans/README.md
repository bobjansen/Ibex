...
| Plan | Notes |
|---|---|
| [kernel-pipeline-execution-plan.md](kernel-pipeline-execution-plan.md) | **Proposed architectural successor for the execution engine.** Separates typed logical IR, physical pipelines, a morsel executor, and a broad precompiled templated kernel library. Explicitly Umbra-like in plan/pipeline structure but **not** a JIT project. Begins with a behavior-preserving `Scan → Filter → Project` vertical slice; existing chunked/multicore plans remain the correctness and measurement inputs. |
