...
| [count-window-plan.md](count-window-plan.md) | Implemented (interpreter) | Per-call count/duration windows work (`__window_n`/`__window_ns` in lower.cpp + window.cpp). Open: codegen support, `window N rows` block syntax, monotonic-deque rolling min/max. **File untracked — commit it** |
