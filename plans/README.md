...
| [deferred-probe-streaming-plan.md](deferred-probe-streaming-plan.md) | Parallel, bounded deferred-probe chunks through row-local right-side wrappers; preserves the Parquet boundary and requires multicore q12 validation. Stage 2 landed (unreachable by any current query); Stage 3 (Filter admission) measured and reverted — fixed q03, broke q12 (no real selectivity, known no-cost-model gap). Blocked on [deferred-probe-selectivity-cost-model-plan.md](deferred-probe-selectivity-cost-model-plan.md). |
