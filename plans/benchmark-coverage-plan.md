...
5. **Pipeline suite Tier 3**: ~~`symbol_day_to_symbol`~~ **DONE** (2026-06-14, commit f7b1b29). ~~`log_return_momentum`~~ **DONE** (2026-06-15, commit e14edd0). Added `prices_ts.csv` (symbol, ts:int64-ns, price) to gen_data.py — purely additive, prices.csv baseline untouched; `as_timeframe` promotes the Int ts → Timestamp so no schema hint/plugin needed. ibex_bench auto-derives the path beside `--csv`. Mirrored with identical semantics across all 7 engines; cross-checked byte-identical per-symbol output. Syntax gotcha: it's `[window 5m, update { mom = rolling_mean(lr) }]` (duration in a `window` clause, not a fn arg). The first return per symbol is null (lag→null); this originally **poisoned the time-windowed rolling_mean → inf**, which surfaced — and was fixed by — null/NaN-aware rolling (commit 6013779: kernels skip nulls / propagate+recover NaN, and grouped windowed update now carries per-group input validity). The query is now the clean `log(price/lag(price,1))` with no coalesce; every engine null-skips in its rolling mean + final mean/std, so output stays byte-identical cross-engine (R needs `na.rm=TRUE`). ibex @16M: ~2.7 s — grouped time-windowed rolling is a future perf target.
