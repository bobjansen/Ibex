...
| Time series | `as_timeframe(timestamp)` promotes Timestamp, Date, or Int64-nanosecond data; `window("5m")` applies to the next native `mutate()` or `summarise()`; rolling helpers and `window_start()` / `window_end()` translate lazily; `resample("1m", close = last(price), .by = symbol)` produces fixed bars | A missing/nullable index, invalid duration, local evaluation of a rolling marker, or resampling without a TimeFrame |
