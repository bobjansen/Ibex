...
auto make_join_probe_operator(OperatorPtr source, std::optional<Table> materialized_source,
    ...
    if (materialized_source.has_value() && exec != nullptr) {
        if (const std::size_t workers = probe_morsel_workers(*materialized_source, *exec);
            workers >= 2) {
            return build_probe_morsel_pipeline(std::move(*materialized_source), probe, workers,
                                               *exec);
        }
    }
