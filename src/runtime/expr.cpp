...
static constexpr auto rng_func_returns_int(std::string_view name) -> bool;
...
static auto eval_date_part(std::string_view name, const ExprValue& v)
...
static auto valid_round_mode(std::string_view m) -> bool {
    return m == "nearest" || m == "bankers" || m == "floor" || m == "ceil" || m == "trunc";
}

static auto extract_ir_round_mode(const ir::Expr& arg)
...
static auto apply_round(double v, std::string_view mode) -> std::int64_t {
    if (mode == "nearest") {
...
static auto extract_rng_param(const ir::CallExpr& call, std::size_t pos, std::string_view func_name)
