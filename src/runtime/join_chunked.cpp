...
struct JoinProbe {
    ...
    auto probe_chunk_against_right(Table left_chunk) -> std::expected<Table, std::string> {
        ...
        if (!columns_.has_value()) {
            return std::unexpected("join column mapping not bound");
        }
        if (pair_mode_) {
...
auto make_chunked_semi_anti_join_operator(OperatorPtr left, Table right,
                                          ir::JoinKind kind,
                                          const std::vector<ir::JoinKey>* keys,
                                          const ExecutionContext* exec) -> OperatorPtr {
    return std::make_unique<ChunkedSemiAntiJoinOperator>(std::move(left), std::move(right),
                                                         kind, keys, exec);
}
