...
[[nodiscard]] auto build_row_local_map_operator(
    const ir::Node& node, OperatorPtr child, const ScalarRegistry* scalars,
    const ExternRegistry* externs, const ExecutionContext& exec, bool preserve_empty_morsels)
...
[[nodiscard]] auto build_map_pipeline_parallel(
    const physical::Plan& plan, const TableRegistry& registry, const ScalarRegistry* scalars,
...
[[nodiscard]] auto build_pipelined_scan(
    const std::vector<MapStep>& operators, bool count_as_pipeline, const DeferredScan& scan,
    std::vector<SourceUnit> units, const ScalarRegistry* scalars, const ExternRegistry* externs,
    const ExecutionContext& exec) -> std::expected<OperatorPtr, std::string>;

[[nodiscard]] auto has_multi_unit_deferred_scan(const ir::Node& node,
