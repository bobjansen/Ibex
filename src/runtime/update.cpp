...
template <typename T>
std::unique_ptr<T[]> make_owned_array(
    std::size_t n) {  // NOLINT(cppcoreguidelines-avoid-c-arrays,modernize-avoid-c-arrays)
    return std::make_unique_for_overwrite<T[]>(
        n);  // NOLINT(cppcoreguidelines-avoid-c-arrays,modernize-avoid-c-arrays)
}
