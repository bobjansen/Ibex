...
// ─── Tuple destructuring → table assignment ───────────────────────────────────

TEST_CASE("E2E: tuple let binding allows destructured names as table expressions", "[e2e]") {
    runtime::Table t;
    t.add_column("price", Column<std::int64_t>{10, 20, 30, 40, 50});
    t.add_column("volume", Column<std::int64_t>{100, 200, 300, 400, 500});
    runtime::TableRegistry tables;
    tables.emplace("trades", std::move(t));
    ...
    // Destructure the two-column table and use price_col as a table expression.
    const char* src = R"(
extern fn get_trades() -> DataFrame from "fake.hpp";
let df = get_trades();
let (price_col, volume_col) = df;
price_col[filter price > 20];
)";
    REQUIRE(ibex::repl::execute_script(src, registry));
}

TEST_CASE("E2E: tuple let binding - filter on second destructured column as table", "[e2e]") {
    runtime::Table t;
    t.add_column("x", Column<std::int64_t>{1, 2, 3, 4, 5});
    t.add_column("y", Column<std::int64_t>{10, 20, 30, 40, 50});
    runtime::TableRegistry tables;
    tables.emplace("data", std::move(t));
    ...
    // Both destructured names should be usable as table expressions.
    const char* src = R"(
extern fn get_data() -> DataFrame from "fake.hpp";
let df = get_data();
let (x_col, y_col) = df;
