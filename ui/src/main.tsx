...
type FileEntry = { name: string; path: string; relative_path: string; directory: boolean };
...
function App() {
  ...
  const refreshFiles = useCallback(async (path = "") => {
    try {
      const response = await api<FileListing>(`/api/v1/files?path=${encodeURIComponent(path)}`);
      setFiles(response); setFilesError(undefined);
    } catch (cause) { setFilesError(cause instanceof Error ? cause.message : "Could not list files"); }
  }, []);
  ...
  const insertFilePath = useCallback((path: string) => {
    ...
    if (!editor) { setSource(current => current + text); return; }
    const selection = editor.getSelection();
    if (selection) editor.executeEdits("file-explorer", [{ range: selection, text, forceMoveMarkers: true }]);
    editor.focus();
  }, []);
  ...
  return <main>
    <aside><header><strong>Environment</strong><button onClick={() => void refreshEnvironment()}>↻</button></header>
      ...
      <section className="file-explorer"><header><strong>Files</strong><button onClick={() => void refreshFiles(files?.path)}>↻</button></header>
        {files?.path && <button className="file-entry parent" onClick={() => void refreshFiles(files.path.split("/").slice(0, -1).join("/"))}>..</button>}
        {files?.entries.map(entry => <button className="file-entry" key={entry.relative_path} onClick={() => entry.directory && void refreshFiles(entry.relative_path)} onDoubleClick={() => !entry.directory && insertFilePath(entry.path)} title={entry.path}>{entry.directory ? "▸ " : ""}{entry.name}</button>)}
